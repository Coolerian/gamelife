...
function startLife(){
    ...
    for (let i=0; i<80; i++){
        ...
        for (let j=0; j<80; j++){
            ...
            if (neighbors==2) ctx.globalAlpha=+0.2;
            if (neighbors>=3) ctx.globalAlpha=+0.3;
           // if (ctx.globalAlpha>=0.6) ctx.fillStyle="black", ctx.globalAlpha=1.0;
            (neighbors==3 || neighbors==2 && ctx.globalAlpha!=0.5) ? mas2[i][j]=1 : mas2[i][j]==0; // если соседей 2 или 3, то
            //оживляем, иначе нет
        }
    }
    mas = mas2;
